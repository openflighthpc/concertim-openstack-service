...
class APIHandler(BaseHandler):
    ...
    def delete_team_role(self, user_id, project_id, role):
        self.__LOGGER.info(f"Deleting role #{role} for user #{user_id} in project #{project_id}")
        try:
            self.openstack_service.remove_user_role_from_project(user_id, project_id, role)
        except Exception as e:
            self.__LOGGER.error(f"Encountered error when removing role {role} for user {user_id} in project {project_id} : {e.__class__.__name__} - {e} - {sys.exc_info()[2].tb_frame.f_code.co_filename} - {sys.exc_info()[2].tb_lineno}")
            raise e
    ...
    def delete_user(self, openstack_user_id):
        self.__LOGGER.info(f"Starting delete of Concertim User")
        try:
            self.openstack_service.delete_user(openstack_user_id)
            return True
        except Exception as e:
            self.__LOGGER.error(f"Encountered error when completing User deletion : {e.__class__.__name__} - {e} - {sys.exc_info()[2].tb_frame.f_code.co_filename} - {sys.exc_info()[2].tb_lineno}")
            raise e
